...
use crate::views::api_health::CallStats;
use crate::views::stamps;
use crate::watch::{
...
fn render_stamps(out: &mut String, batches: &[PostageBatch]) {
    ...
    let rows = stamps::rows_for(&StampsSnapshot {
...
fn render_pending_tx(out: &mut String, pending: &[bee::debug::TransactionInfo], now_unix: i64) {
    ...
    let oldest = pending
        .iter()
        .filter_map(|t| {
            crate::views::api_health::parse_rfc3339_to_unix(&t.created).map(|ts| now_unix - ts)
        })
        .max()
        .unwrap_or(0);
