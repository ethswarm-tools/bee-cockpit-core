//! S7 — Network / NAT view. Pure half of bee-tui's
//! `components::network`: the AutoNAT reachability ladder
//! (`ReachabilityStatus`, `AvailabilityStatus`), per-underlay
//! public/private classification (`UnderlayKind`, `classify_multiaddr`
//! + ipv4/ipv6 helpers), and the stability formatter. The renderer
//! owns the watch subscriptions, the rolling stability-window timer,
...
/// Tri-state for `networkAvailability`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Per-underlay public/private classification.
...
pub struct NetworkView {
    /// Short form (first 6 + last 4 hex). Use in tooltips / counters;
    /// the renderer favours the full form below so operators can
    /// copy via terminal click-drag.
...
/// loopback ranges count as Private; DNS multiaddrs are Unknown
/// because we can't tell without resolving.
...
/// Format a stability duration as `Xh Ym`, `Xm Ys`, or `Xs`. Lives
/// here so renderers share the format; the actual duration is
/// computed renderer-side because it depends on wall-clock state
/// the snapshot tests can't pin.
