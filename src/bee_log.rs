...
/// Bee-side log tabs the cockpit's bottom pane shows. Defined here
/// (rather than in `bee-tui`'s `components::log_pane`) so the parser
/// and tailer in core can reference them without depending on the
/// renderer.
...
/// builds these from parsed Bee log entries.
